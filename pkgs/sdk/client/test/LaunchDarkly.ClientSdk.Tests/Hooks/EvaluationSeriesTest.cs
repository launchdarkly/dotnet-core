...
namespace LaunchDarkly.Sdk.Client.Hooks
{
    ...
    public class EvaluationSeriesTest : BaseTest
    {
        ...
        public void HooksAreExecutedInLifoOrder(string[] hookNames, string[] executions)
        {
            ...
            var context = new EvaluationSeriesContext("flag", Context.New("test"), LdValue.Null,
                Method.BoolVariation);

            var executor = new Executor(testLogger, hookNames.Select(name => new SpyHook(name, got)));

            executor.EvaluationSeries(context, LdValue.Convert.Bool, () => new EvaluationDetail<bool>());
        ...
        public void StageFailureLogsExpectedMessages(string flagName, string hookName, string beforeError,
            string afterError)
        {
            var hooks = new List<Hook>
            {
            ...
            var context = new EvaluationSeriesContext(flagName, Context.New("test"), LdValue.Null,
                Method.BoolVariation);
